        ...
        if args["set_type"] == "train":
            self.transform = Compose(
                [
                    ...
                    RandomCrop(height=resize[0], width=resize[1], p=1.0),
                    FancyPCA(p=1.0),
            ...
            self.transform = Compose(
                [
                    Sharpen(p=1.0),
                    Resize(256, 256, p=1.0),
                    RandomCrop(height=resize[0], width=resize[0], p=1.0),
                    FancyPCA(p=1.0),
                    # Normalize(mean=mean, std=std),
                    ToTensorV2(),
                ]
...
class MaskBaseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # devide 255 for normalization (min max normalization)
        return (
            image_transform / 255.0,
...
class TestDataset(Dataset):
    ...
    def calc_statistics(self):
        ...
        if not has_statistics:
            ...
            sums = []
            squared = []
            for image_path in self.img_paths[:3000]:
                image = np.array(Image.open(image_path)).astype(np.int32)
                sums.append(image.mean(axis=(0, 1)))
                squared.append((image**2).mean(axis=(0, 1)))
    ...
    def __getitem__(self, index):
        ...
        if self.transform:
            image = self.transform(image=np.array(face_image))["image"]

        # devide 255 for min max normalization
        return image / 255.0
...
if __name__ == "__main__":
    ...
    train_transform = BaseAugmentation(
        resize=(224, 224),
        mean=(0.548, 0.504, 0.479),
        std=(0.237, 0.247, 0.246),
        set_type="train",
    ...
    val_transform = BaseAugmentation(
        resize=(224, 224),
        mean=(0.548, 0.504, 0.479),
        std=(0.237, 0.247, 0.246),
        set_type="val",
    ...
    _, val_set = valid_dataset.split_dataset()
    ...
    dataset = TestDataset(img_paths, (256, 256))
