...
def apply_transforms(
    ...
    if "train" in need:
        transformations["train"] = A.Compose(
            [
                A.CenterCrop(img_size[0], img_size[1], p=1.0),
                A.HorizontalFlip(p=0.4),
                A.HueSaturationValue(
                    hue_shift_limit=0.2, sat_shift_limit=0.2, val_shift_limit=0.2, p=0.4
                ),
                A.RandomBrightnessContrast(
                    brightness_limit=(-0.1, 0.1), contrast_limit=(-0.1, 0.1), p=0.4
                ),
                A.ColorJitter(
                    brightness=(0.8, 1.2),
                    contrast=(0.8, 1.2),
                ...
                A.GaussianBlur(blur_limit=(3, 9), sigma_limit=(0.0, 0), p=0.4),
                A.GaussNoise(p=0.4),
    ...
    if "valid" in need:
        transformations["valid"] = A.Compose(
            [
                A.CenterCrop(img_size[0], img_size[1], p=1.0),
                A.Normalize(mean=mean, std=std, max_pixel_value=255.0, p=1.0),
                ToTensorV2(p=1.0),
...
train_dir = "../input/data/train"  # ! Train Dataset에 대한 Path Setting
train_img_dir = f"{train_dir}/images"
train_df_path = f"{train_dir}/train.csv"
train_df = pd.read_csv(train_df_path)

test_dir = "../input/data/eval"  # ! Test Dataset에 대한 Path Setting
...
# ! Image에 대해 Random하게 Train / Valid Split 수행한 Dataset Class
class BaseMaskDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return image_transform, (
            ...
            gender_label,
            age_label,
        )  # TODO { Image, (Mask, Gender, Age) } 로 반환
...
# ! 사람에 대해 Random하게 Train / Valid Split 수행한 Dataset Class
class AdvancedMaskDataset(BaseMaskDataset):
    """
...
class TestDataset(Dataset):
    def __init__(
        ...
        self.transform = transforms.Compose(
            [
                transforms.CenterCrop(size=(resize[0], resize[1])),
                transforms.ToTensor(),
