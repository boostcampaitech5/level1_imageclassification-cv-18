...
def train(data_dir, model_dir, args):
    ...
    for train_index, val_index in kfold.split(dataset):
        ...
        # -- loss & metric
        if args.criterion == "f1" and "MultiLabel" in args.model:
            mask_criterion = create_criterion(args.criterion, classes=3)
            gender_criterion = create_criterion(args.criterion, classes=2)
            age_criterion = create_criterion(args.criterion, classes=3)
        else:
            mask_criterion = create_criterion(args.criterion)
            gender_criterion = create_criterion(args.criterion)
            ...
            for idx, train_batch in enumerate(train_loader):
                ...
                with torch.cuda.amp.autocast():
                    if "MultiLabel" in args.model:
                        ...
                        mask_loss = mask_criterion(mask_out, mask_label)
                        ...
                        loss = mask_criterion(outs, labels)
            ...
            with torch.no_grad():
                ...
                for val_batch in val_loader:
                    ...
                    if "MultiLabel" in args.model:
                        ...
                        outs = model(inputs)
                        preds = torch.argmax(outs, dim=-1)
                        loss_item = mask_criterion(outs, labels).item()

                    acc_item = (labels == preds).sum().item()
                    # mask_acc_item = (mask_label == mask_pred).sum().item()
                    # gender_acc_item = (mask_label == mask_pred).sum().item()
                    # age_acc_item = (mask_label == mask_pred).sum().item()
                ...
                if val_acc > best_val_acc:
                    ...
                    torch.save(model.module.state_dict(), f"{save_dir}/best{index}.pth")
                    best_val_acc = val_acc
                torch.save(model.module.state_dict(), f"{save_dir}/last{index}.pth")
                print(
